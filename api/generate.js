...
Devuelve EXACTAMENTE este JSON completo:
{
  ...
  "bonuses": [
    {"name":"NOMBRE DEL BONO","desc":"descripción breve del valor","value":"USD 47"},
    {"name":"NOMBRE DEL BONO","desc":"descripción breve del valor","value":"USD 37"},
    {"name":"NOMBRE DEL BONO","desc":"descripción breve del valor","value":"USD 27"}
  ],
  "trust_badges": [
...
Incluye: 3 titulos magneticos, indice con 7 capitulos, CAPITULO 1 desarrollado (600 palabras), CAPITULO 2 desarrollado (600 palabras), CAPITULO 3 desarrollado (500 palabras), conclusion con CTA (300 palabras).`,
        bonos: `Eres experto en lanzamientos. Crea 5 bonos irresistibles en español para:
Producto: ${data.product}, Publico: ${data.audience}, Precio: ${data.price}.
Para cada bono: nombre creativo, descripcion 3-4 lineas, valor percibido USD, justificacion emocional, copy para presentarlo. Al final tabla resumen valor total vs precio.`,
...
function buildPremiumLanding(lc, data, c) {
  ...
  const paraVosHTML = (c.para_vos_si || []).map(i => `
    <div class="pv-item">
      ...
      <div class="pv-body">
        <div class="pv-title">${i.title}</div>
  ...
  const aprendHTML = (c.que_vas_aprender || []).map(i => `
    <div class="learn-item">
      ...
      <div class="learn-body">
        <div class="learn-title">${i.title}</div>
        <div class="learn-desc">${i.desc}</div>
      </div>
    </div>`).join('');

  const bonusHTML = (c.bonuses || []).map((b, i) => `
    <div class="bonus-item">
      <div class="bonus-num">0${i+1}</div>
      <div class="bonus-body">
        <div class="bonus-name">${b.name}</div>
        <div class="bonus-desc">${b.desc}</div>
        <div class="bonus-val">Valor: <s>${b.value}</s> → <span>GRATIS hoy</span></div>
      </div>
    </div>`).join('');
...
.divider{display:flex;align-items:center;gap:10px;margin:20px auto;max-width:300px}
.divider-line{flex:1;height:1px;background:var(--c1);opacity:.3}
.divider-dot{width:5px;height:5px;border-radius:50%;background:var(--c1);opacity:.6}
...
/* HERO */
.hero{padding:60px 20px 40px;text-align:center;position:relative;overflow:hidden;background:radial-gradient(ellipse 100% 80% at 50% 0%,rgba(212,175,55,.08),transparent 60%),var(--bg)}
.hero-orbs{position:absolute;inset:0;pointer-events:none;animation:orbsMove 8s ease-in-out infinite}
@keyframes orbsMove{0%,100%{box-shadow:0 0 80px 30px rgba(212,175,55,.06),200px 100px 100px 40px rgba(212,175,55,.04),-100px 150px 80px 30px rgba(212,175,55,.05)}50%{box-shadow:100px 50px 100px 40px rgba(212,175,55,.08),-80px 80px 80px 30px rgba(212,175,55,.06),50px 200px 100px 40px rgba(212,175,55,.04)}}
...
/* IMG PLACEHOLDER */
.img-placeholder{background:rgba(255,255,255,.04);border:2px dashed rgba(212,175,55,.3);border-radius:16px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:40px 20px;text-align:center;color:rgba(212,175,55,.5);font-size:13px;font-weight:600;letter-spacing:1px;min-height:280px}
.img-placeholder .ph-icon{font-size:2.5rem;opacity:.4}
.img-placeholder .ph-text{text-transform:uppercase;letter-spacing:2px}
.img-placeholder .ph-sub{font-size:11px;opacity:.6;font-weight:400;letter-spacing:1px;margin-top:4px}

/* MOCKUP SECTION */
.mockup-section{padding:60px 20px;text-align:center;background:linear-gradient(180deg,var(--bg),rgba(212,175,55,.03))}
.mockup-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:32px;max-width:700px;margin-left:auto;margin-right:auto}
...
.pv-tagline{text-align:center;font-size:1rem;font-weight:800;letter-spacing:1px;text-transform:uppercase;margin:8px 0 32px;opacity:.7}
...
.learn-title{text-align:center;font-size:clamp(1.8rem,6vw,2.8rem);font-weight:900;margin-bottom:8px}
.learn-subtitle{text-align:center;font-size:clamp(1.4rem,5vw,2.2rem);font-weight:800;margin-bottom:32px}
...
.learn-img-wrap{margin:32px auto 0;max-width:500px}
...
.cd-divider{display:flex;align-items:center;gap:10px;margin:20px auto;max-width:200px}
.cd-divider-line{flex:1;height:1px;background:var(--c1);opacity:.25}
.cd-divider-dot{width:5px;height:5px;border-radius:50%;background:var(--c1);opacity:.5}
...
.sold-bar{height:16px;background:rgba(255,255,255,.08);border-radius:100px;overflow:hidden;position:relative}
.sold-fill{height:100%;background:linear-gradient(90deg,var(--c2),var(--c1));border-radius:100px;animation:fillBar 2.5s ease forwards;position:relative}
.sold-fill::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,.3),transparent);animation:shimmer 2s infinite}
...
.bonus-list{display:flex;flex-direction:column;gap:12px;max-width:680px;margin:0 auto 32px}
.bonus-item{display:flex;align-items:flex-start;gap:16px;background:linear-gradient(135deg,rgba(212,175,55,.08),rgba(212,175,55,.03));border:1px solid rgba(212,175,55,.2);border-radius:16px;padding:20px}
.bonus-item:hover{border-color:var(--c1);transform:translateX(6px);transition:all .25s}
.bonus-num{font-size:2rem;font-weight:900;color:var(--c1);min-width:50px;text-align:center;opacity:.8}
.bonus-name{font-size:.95rem;font-weight:800;color:#fff;margin-bottom:4px;letter-spacing:.5px}
.bonus-desc{font-size:.82rem;color:var(--muted);margin-bottom:6px;line-height:1.5}
.bonus-val{font-size:.78rem;font-weight:700;color:var(--c1)}
.bonus-val s{color:var(--muted);margin-right:4px}
.bonus-val span{color:#22c55e}
.bonus-img-wrap{max-width:500px;margin:32px auto 0}
...
.wa-note{text-align:center;font-size:.82rem;color:var(--muted);margin-top:16px}
...
.price-box-header{background:var(--c2);padding:20px 24px;text-align:center;position:relative;overflow:hidden}
...
.tk-card{background:radial-gradient(circle at 20% 10%,rgba(212,175,55,.14),transparent 45%),radial-gradient(circle at 80% 70%,rgba(15,61,46,.22),transparent 45%),linear-gradient(180deg,#060707,#000);border:1px solid rgba(212,175,55,.28);border-radius:16px;padding:12px;display:flex;gap:12px;align-items:center;box-shadow:0 18px 46px rgba(0,0,0,.55)}
.tk-avatar{width:50px;height:50px;border-radius:50%;overflow:hidden;flex:0 0 auto;border:2px solid rgba(15,61,46,.95);box-shadow:0 0 0 2px rgba(212,175,55,.22);background:#111;position:relative}
...
.tk-close{border:0;background:transparent;color:rgba(212,175,55,.95);cursor:pointer;font-size:20px;line-height:1;padding:0 6px;opacity:.9}

@keyframes marqueeScroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
@media(max-width:600px){
  .hero-h1{font-size:2rem}
  .cd-digit{width:60px;height:60px;font-size:32px}
  ...
  <div class="container">
    <div class="hero-content">
      ...
      <h1 class="hero-h1">
        ...
        <span class="line2">${c.headline2 || ''}</span>
      </h1>
...
<section class="mockup-section">
  <div class="container">
    <p style="font-size:11px;letter-spacing:5px;text-transform:uppercase;color:var(--c1);opacity:.8;margin-bottom:8px">Tu producto</p>
    <h2 style="font-size:clamp(1.5rem,5vw,2.2rem);font-weight:900;margin-bottom:8px">Conocé lo que recibís</h2>
    <div class="mockup-grid">
      <div class="img-placeholder">
        <div class="ph-icon">📦</div>
        <div class="ph-text">Mockup del Producto</div>
        <div class="ph-sub">Agregá imagen del producto aquí</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:12px;text-align:left;justify-content:center">
        ${(c.que_vas_aprender || []).slice(0,4).map(i => `
        <div style="display:flex;align-items:flex-start;gap:10px">
          <span style="color:var(--c1);font-weight:800;flex-shrink:0">✓</span>
          <div>
            <div style="font-size:.82rem;font-weight:800;color:var(--c1);letter-spacing:1px">${i.title}</div>
            <div style="font-size:.78rem;color:var(--muted)">${i.desc}</div>
          </div>
        </div>`).join('')}
      </div>
    </div>
...
<section class="learn-section">
  <div class="container">
    ...
    <h2 class="learn-title">¿QUÉ VAS A</h2>
    <h2 class="learn-subtitle"><span class="gold">APRENDER PASO A PASO?</span></h2>
    <div class="learn-grid">${aprendHTML}</div>
    <div class="learn-img-wrap">
      <div class="img-placeholder">
        <div class="ph-icon">📚</div>
        <div class="ph-text">Imagen "Lo que vas a aprender"</div>
        <div class="ph-sub">Generá con ChatGPT y pegá la URL aquí</div>
      </div>
    </div>
  </div>
</section>
...
<section style="padding:40px 20px;background:rgba(0,0,0,.1)">
  <div class="countdown-section">
    <div class="cd-wrap">
      ...
      <div class="cd-body">
        <p class="cd-desc">Este precio especial expira en <strong>10 minutos</strong>.<br>Después puede aumentar sin previo aviso.</p>
        ...
        <div class="cd-divider"><div class="cd-divider-line"></div><div class="cd-divider-dot"></div><div class="cd-divider-line"></div></div>
...
<section class="bonus-section">
  <div class="container">
    ...
    <h2 class="bonus-title">BONOS <span class="gold">EXCLUSIVOS</span> INCLUIDOS</h2>
    <p class="bonus-sub">Al comprar hoy, recibís estos bonos completamente gratis.</p>
    <div class="bonus-list">${bonusHTML}</div>
    <div class="bonus-img-wrap">
      <div class="img-placeholder">
        <div class="ph-icon">🎁</div>
        <div class="ph-text">Imagen de Bonos</div>
        <div class="ph-sub">Generá con ChatGPT y pegá la URL aquí</div>
      </div>
    </div>
...
<section class="testi-section">
  <div class="container">
    ...
    <p class="wa-note">📱 ¿Tenés capturas de WhatsApp de clientes reales? Reemplazá los testimonios con tus imágenes.</p>
  </div>
</section>
    ...
    <div class="price-box">
      ...
      <div class="price-box-body">
        ...
        <div class="price-includes">
          ...
          ${(c.bonuses||[]).map(b=>`<div class="price-inc-item">Bono: ${b.name}</div>`).join('')}
          <div class="price-inc-item">Acceso inmediato al descargar</div>
          <div class="price-inc-item">Soporte personalizado incluido</div>
        </div>
      </div>
    </div>
  </div>
...
<section class="guarantee-section">
  <div class="container" style="max-width:600px">
    ...
    <p class="guarantee-text">${c.guarantee_text || `Si en ${guarantee} días no estás satisfecha con el producto, te devolvemos el 100% de tu dinero. Sin preguntas, sin complicaciones. Así de seguros estamos de que va a transformar tu vida.`}</p>
  </div>
</section>
...
<section class="faq-section">
  <div class="container">
    ...
    <p class="faq-sub">Todo lo que necesitás saber antes de acceder a ${data.product}.</p>
    <div class="faq-card">
      ${faqHTML}
...
function updateViewers(){
  var v=Math.floor(Math.random()*28)+14;
  var el1=document.getElementById('viewers');
  var el2=document.getElementById('viewers2');
...
function tick(){
  ...
  var me=document.getElementById('cd-min'),se=document.getElementById('cd-sec');
...
// Popup FOMO
var pops=[
  {name:"Luciana",place:"Buenos Aires 🇦🇷",img:"https://randomuser.me/api/portraits/women/44.jpg"},
  {name:"Marcos",place:"Rosario 🇦🇷",img:"https://randomuser.me/api/portraits/men/32.jpg"},
  {name:"Valentina",place:"Montevideo 🇺🇾",img:"https://randomuser.me/api/portraits/women/68.jpg"},
  {name:"Carlos",place:"Bogotá 🇨🇴",img:"https://randomuser.me/api/portraits/men/46.jpg"},
  {name:"Fernanda",place:"Ciudad de México 🇲🇽",img:"https://randomuser.me/api/portraits/women/29.jpg"},
  {name:"Diego",place:"Santiago 🇨🇱",img:"https://randomuser.me/api/portraits/men/55.jpg"},
  {name:"Camila",place:"Lima 🇵🇪",img:"https://randomuser.me/api/portraits/women/15.jpg"},
  {name:"Rodrigo",place:"Asunción 🇵🇾",img:"https://randomuser.me/api/portraits/men/22.jpg"},
  {name:"Sofía",place:"Córdoba 🇦🇷",img:"https://randomuser.me/api/portraits/women/63.jpg"},
  {name:"Andrés",place:"Medellín 🇨🇴",img:"https://randomuser.me/api/portraits/men/41.jpg"}
];
var actions=[${popupActionsJS}];
var times=["hace 1 min","hace 2 min","hace 3 min","hace 5 min","ahora mismo","hace 7 min"];
var pi=0;
function showPopup(){
  var p=pops[pi%pops.length];pi++;
  var a=actions[Math.floor(Math.random()*actions.length)];
  var t=times[Math.floor(Math.random()*times.length)];
  var el=document.getElementById('tkFomo');
  document.getElementById('tkAvatar').src=p.img;
  document.getElementById('tkPlace').textContent=p.place;
  document.getElementById('tkWhen').textContent=t;
  document.getElementById('tkTitle').innerHTML='<b>'+p.name+'</b> de <b>'+p.place+'</b> '+a;
  el.classList.add('show');
  setTimeout(function(){el.classList.remove('show');},5000);
}
setTimeout(showPopup,3500);
setInterval(showPopup,14000);
</script>
</body>
</html>`;
}
